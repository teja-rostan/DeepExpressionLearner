...
class CNNLearner:
    ...
    def __init__(self, class_size, n_hidden_neurons=30, conv_type="class"):
        ...
        self.n_kernels = 12
        self.k = 6
        self.final_image_size = 1236  # manual fast fix
        ...
        if conv_type == "reg":
            ...
            # self.noise_py_x = nnet.conv_model(X, self.w_h, self.w_h2, self.w_h3, self.w_h4, self.w_h5, self.w_o, 0, 0)
            # self.py_x = nnet.conv_model(X, self.w_h, self.w_h2, self.w_h3, self.w_h4, self.w_h5, self.w_o, 0., 0.)

            # self.noise_py_x = nnet.conv_model2(X, self.w_h, self.w_h2, self.w_h4, self.w_o, 0.0, 0.0)
            # self.py_x = nnet.conv_model2(X, self.w_h, self.w_h2, self.w_h4, self.w_o, 0., 0.)

            self.noise_py_x = nnet.conv_model3(X, self.w_h, self.w_h4, self.w_h5, self.w_o, 0.0, 0.0)
            self.py_x = nnet.conv_model3(X, self.w_h, self.w_h4, self.w_h5, self.w_o, 0., 0.)
        ...
        # params = [self.w_h, self.w_h2, self.w_h4, self.w_o]
        params = [self.w_h, self.w_h4, self.w_o]
